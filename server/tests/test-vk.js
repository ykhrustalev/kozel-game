...
module.exports = {
  ...
  handleAuth: function (test) {
    ...
    referers.valid.forEach(function (r) {
      test.ok(handler.canHandle(getMockRequest(r)));
      handler.handle(getMockRequest(r), function (error, accepted, profile) {
        test.ok(!error);
        test.ok(accepted);
        test.ok(profile);
        test.ok(profile.uid);
    ...
    referers.cantHandle.forEach(function (r) {
      test.ok(!handler.canHandle(getMockRequest(r)));
      count++;
    });
    ...
    // async guard
    var requiredCount = referers.valid.length + referers.invalid.length + referers.cantHandle.length;
